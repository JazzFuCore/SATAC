...
function processaCoordenadas($coordenadas) {
    ...
    <h1>PESQUISAR</h1>
		...
		$sql = "SELECT COUNT(*) FROM `contador_com_telemetria` WHERE id LIKE '%{$qrs}%' OR nome LIKE '%{$qrs}%' OR num_contador LIKE '%{$qrs}%'";
	...
	<form action="" method="GET">
		...
		<input type="text" id="texto" name="qrs" style="width: 45%;" placeholder="Escreva aqui para pesquisar" value='<?php if(isset($_GET['qrs']))echo $_GET['qrs']; ?>' >
	</form>
	<center class="links">
		<?php 
    ...
    <table border="3">
        <thead>
            <tr>
                ...
                <th>Perdas [7 dias registados] (<a href="#" class="popup-trigger">2</a>)</th>
                <th>Classificação [60 dias registados] (2)</th>
				<th>Consumo Médio Diário (m3/dia) (<a href="#" class="popup-trigger3">3</a>)</th>
                <th>Fuga Dia (m3/dia) (3)</th>
                <th>% de perdas de água (3)</th>
            </tr>
        </thead>
        <tbody>
			<?php
			...
			if(array_key_exists("qrs", $_GET)){
				...
				$offset =  $linhas_pag * ( $pag_s - 1 );
					...
					WHERE id LIKE '%{$qrs}%' OR nome LIKE '%{$qrs}%' OR num_contador LIKE '%{$qrs}%'
...
<br>
	...
	<script>
		document.addEventListener("DOMContentLoaded", function() {
			const trigger = document.querySelector(".popup-trigger3");
			const popup = document.getElementById("popup3");
			const closeBtn = document.getElementById("popup-close3");
